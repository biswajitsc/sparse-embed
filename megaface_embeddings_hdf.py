...
flags.DEFINE_string(name='model',
                    ...
                    default='mobilenet')
flags.DEFINE_string(name='output_file',
...
def input_fn(filelist):
  ...
  base_dir = tf.constant(FLAGS.data_dir + '/')

  def _parse_function(filename, label):
    image_string = tf.read_file(tf.strings.join([base_dir, filename]))
...
def main(_):
  ...
  data = h5py.File(os.path.join('embeddings', FLAGS.output_file), mode='w')
  ...
  for prediction in tqdm.tqdm(predictions):
    ...
    idxs = np.where(np.abs(embedding) >= 1e-8)[0]
    ...
    step += 1
    tot_embeddings[0] = step
    avg = (1 - 1/step) * avg + len(idxs) / step
    if step % 10000 == 0:
